...
export default function RentalCarDetail() {
  ...
  useEffect(() => { API.get(`/rentals/${id}`).then(({ data }) => { setRental(data); setActiveImage(0); }).catch(() => setRental(false)); }, [id]);
  if (rental === null) return <p className="detail-loading">Loading rental car...</p>;
  if (!rental) return <p className="detail-loading">Rental car not found.</p>;
  const seller = rental.seller?.businessName || rental.seller?.name || 'Seller';

  return <main className="car-detail-wrapper rental-detail"><Link className="detail-back-btn" to="/cars-for-rent">← Back to rentals</Link>
    <div className="detail-image-wrap">{rental.images?.length ? <img src={imageUrl(rental.images[activeImage])} alt={rental.makeModel} /> : <div className="detail-image-placeholder">🚗</div>}</div>
    {rental.images?.length > 1 && <div className="detail-thumbnail-row">{rental.images.map((image, index) => <img className={`detail-thumbnail ${index === activeImage ? 'active' : ''}`} key={image} src={imageUrl(image)} alt={`${rental.makeModel} ${index + 1}`} onClick={() => setActiveImage(index)} />)}</div>}
...
function Spec({ label, value }) { return <div className="detail-meta-card"><div className="detail-meta-label">{label}</div><div className="detail-meta-value">{value}</div></div>; }
