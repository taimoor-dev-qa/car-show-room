...
export default function CarsForRent() {
  ...
  return <><header><div className="container"><Link to="/" className="logo">Car<span>Zone</span></Link><nav><ul className="nav-list"><li><Link to="/">Browse Cars</Link></li><li><Link to="/cars-for-rent" className="active">Cars for Rent</Link></li>{user ? <><li><Link to="/my-chats">My Chats</Link></li><li><button type="button" className="logout-link" onClick={logout}>Logout</button></li></> : <li><Link to="/login" className="seller-link">Login</Link></li>}</ul></nav></div></header>
