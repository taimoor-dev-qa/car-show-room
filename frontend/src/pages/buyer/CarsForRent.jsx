...
export default function CarsForRent() {
  ...
  const [filters, setFilters] = useState({ city: '', category: 'All', minPrice: '', maxPrice: '', driverAvailable: false });
  const [loading, setLoading] = useState(true);
  const fetchRentals = async () => { setLoading(true); try { const { driverAvailable, ...params } = filters; if (driverAvailable) params.driverAvailable = 'true'; const { data } = await API.get('/rentals', { params }); setRentals(data); } finally { setLoading(false); } };
  useEffect(() => { fetchRentals(); }, []);
  const change = (event) => setFilters({ ...filters, [event.target.name]: event.target.type === 'checkbox' ? event.target.checked : event.target.value });
