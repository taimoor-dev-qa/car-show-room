...
const initialForm = () => {
  ...
  availableUntil.setMonth(
    availableUntil.getMonth() + 2
  );
...
export default function AddRentalCar() {
  ...
  useEffect(() => {
    ...
    if (editId) {
      API.get(`/rentals/${editId}`)
        .then(({ data }) => {
          setForm({
            ...initialForm(),
            ...data,
            availableFrom: dateValue(data.availableFrom),
            availableUntil: dateValue(data.availableUntil),
          });

          setExistingImages(data.images || []);
        })
        .catch(() => {
          setError('Unable to load rental listing.');
        });
    }
  ...
  const selectImages = (event) => {
    ...
    if (
      ...
      setError(
        'A rental listing can have at most 10 images. Remove an existing image or select fewer files.'
      );
      ...
      <main className="main-content">
        <header className="top-header">
          <div className="page-heading">
            ...
            <p>
              Create a separate rental listing for your fleet.
            </p>
          </div>
        </header>

        <div className="rental-form-card content-card">
          <form
          ...
          >
            ...
            {editId &&
              existingImages.length > 0 && (
                <div className="rental-existing-images">
                  <label>
                    Current Images
                  </label>

                  <div className="rental-image-grid">
                    {existingImages.map(
                      (image) => (
                        <div
                          className="rental-image-thumb"
                          key={image}
                        >
                          <img
                            src={imageUrl(image)}
